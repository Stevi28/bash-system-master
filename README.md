...
```bash
   0 0 * * * /bin/bash /path/to/bash-system-master/system_health.sh
```
...
```
bash-system-master/
├── system_health.sh      # System health monitor
...
- Bash 4.0+
- Standard Linux/macOS utilities: `df`, `vm_stat` or `free`, `ps`, `awk`, `tar`, `zip`
- `zip` package (for zip backups on Linux): `sudo apt install zip`